...
class Data:
    def __init__(self, dataset, value_fn, normalize_value=True):
        ...
        self.num_feats, self.states, self.actions, self.next_states, self.entropies, self.dones, self.values, self.rewards, self.policies, self.task_critic_vals, self.safety_critic_vals = self.parse_data()
    ...
    def parse_data(self):
        ...
        for e in self.dataset:
            if e['dones'][-1]: #if episode ends on a done
                ...
                # and add it to dictionary e, follow how it doing as entropy
                #random.append(np.squeeze(e['random']))
                done.append(np.squeeze(e['dones']))
                reward.append(np.squeeze(e['rewards']))
                policies.append(np.squeeze(e['policies']))
                task_critic_vals.append(np.squeeze(e['task_critic_vals']))
                safety_critic_vals.append(np.squeeze(e['safety_critc_vals']))
            else:
                ...
                #random.append(np.squeeze(e['random'])[:-1])
                done.append(np.squeeze(e['dones'])[:-1])
                reward.append(np.squeeze(e['rewards'])[:-1])
                policies.append(np.squeeze(e['policies'])[:-1])
                task_critic_vals.append(np.squeeze(e['task_critic_vals'])[:-1])
                safety_critic_vals.append(np.squeeze(e['safety_critc_vals'])[:-1])
        ...
        #rd = np.array(random[0])
        d = np.array(done[0])
        r = np.array(reward[0])
        plc = np.array(policies[0])
        tcv = np.array(task_critic_vals[0])
        scv = np.array(safety_critic_vals[0])
        ...
        for i in range(len(states)-1):
            if np.array(actions[i+1]).ndim == 0:
                ...
                policies[i+1] = [policies[i+1]]
                task_critic_vals[i+1] = [task_critic_vals[i+1]]
                safety_critic_vals[i+1] = [safety_critic_vals[i+1]]
            ...
            plc = np.concatenate([plc, np.array(policies[i+1])], axis=0)
            tcv = np.concatenate([tcv, np.array(task_critic_vals[i+1])], axis=0)
            scv = np.concatenate([scv, np.array(safety_critic_vals[i+1])], axis=0)
        ...
        #random = np.reshape(e, [-1])
        done = np.reshape(d, [-1])
        reward = np.reshape(r, [-1])
        policies = np.reshape(plc, [-1])
        task_critic_vals = np.reshape(tcv, [-1])
        ...
        return num_feats, states, actions, next_states, entropy, done, values, reward, policies, task_critic_vals, safety_critic_vals
