...
class APG:
    ...
    def compute_graph_info(self, c, take_randoms = False):
        transitions = np.zeros([len(c)+1, len(c)+1])
        actions = [[[] for i in range(len(c)+1)] for j in range(len(c) + 1)]
        randoms = [[[] for i in range(len(c)+1)] for j in range(len(c) + 1)]
        policies = [[[] for i in range(len(c)+1)] for j in range(len(c) + 1)]
        task_critic_vals = [[[] for i in range(len(c)+1)] for j in range(len(c) + 1)]
        safety_critic_vals = [[[] for i in range(len(c)+1)] for j in range(len(c) + 1)]
        ...
        for i in range(len(c)):
            for t in c[i]:
                ...
                randoms[i][n].append(random)
                policies[i][n].append(policy)
                task_critic_vals[i][n].append(task_critic_val)
                safety_critic_vals[i][n].append(safety_critic_val)
        
        if not take_randoms:
          return self.lookup, transitions, actions, policies, task_critic_vals, safety_critic_vals #, randoms
        else: # rss-comment - if-else added to be consistent with the apg_baseline.compute_graph_info() in caps.py
          return self.lookup, transitions, actions, randoms, policies, task_critic_vals, safety_critic_vals
