...
class GaussianPolicy(nn.Module):
    ...
    def sample(self, state):
        ...
        entropy = -normal.log_prob(x_t).sum(-1)
        return action, log_prob, mean, std, entropy
...
class StochasticPolicy(nn.Module):
    ...
    def sample(self, state):
        ...
        action = dist.rsample()
        entropy = -dist.log_prob(action).sum(-1)
        return action, dist.log_prob(action).sum(-1), dist.mean, dist.stddev, entropy
