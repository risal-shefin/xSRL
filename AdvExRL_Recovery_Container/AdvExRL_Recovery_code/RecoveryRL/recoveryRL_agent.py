...
class Recovery_SAC(object):
    ...
    def select_action(self, state, eval=False, ret_entropy=False):
        '''
        ...
        if self.use_constraint_sampling:
            ...
            pi, log_pi, entropy = self.policy.sample(state_batch)
        ...
        # Action Sampling for all other algorithms
            ...
            if eval is False:
                action, entropy, _ = self.policy.sample(state)
            else:
                _, entropy, action = self.policy.sample(state)
    ...
    def load_models(self, expert_path, recovery_path):
        critic_path = os.path.join(expert_path,'critic.pth')
        policy_path = os.path.join(expert_path,'policy.pth')
        print("-------\n load_models() of recoveryRL_agent.py -------\n")
        print("Recovery Critic Path", critic_path)
        print("Recovery Policy Path", policy_path)

        self.critic.load(critic_path)
        self.policy.load(policy_path)
        grad_false(self.critic)
        grad_false(self.critic)
        self.safety_critic.load_rec_model(recovery_path)
